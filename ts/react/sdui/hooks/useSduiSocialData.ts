...
const useSduiSocialData = (pageContext: TSduiPageContext): TSduiSocialData => {
  ...
  const onPresenceUpdate = useCallback(
    (event: Event): void => {
      ...
      const updatedInGameFriendsByUniverseId = getInGameFriendsByUniverseIdFromFriendDetails(
  ...
  useEffect(() => {
    ...
    const getFriendDetails = async (): Promise<void> => {
      try {
        ...
        const initialInGameFriendsByUniverseId = getInGameFriendsByUniverseIdFromFriendDetails(
          friendDetails
        );
