...
const RegexBasedTraceSamplerSettings: Record<string, { pattern: RegExp; sampleRate: number }> = {
...
export function buildSampleRate(baseRate: number) {
  ...
  const regexSampleRate = Object.values(RegexBasedTraceSamplerSettings).find(setting =>
