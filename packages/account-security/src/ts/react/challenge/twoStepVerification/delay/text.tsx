...
export const calculateDelayOffset = (
  { timestamp, dayTranslation, hourTranslation, minuteTranslation }:
  calculateDelayOffsetProps
): DelayOffset | undefined => {
...
export const getDelayTextFromDates = (
  ...
  if (hardCodedFrictionTypesFallback.some(eligibleMethod => 
    eligibleMethod.method === frictionType && 
    eligibleMethod.bypassable)) {
...
export const getDelayBodyTextFromDates = (
  ...
  { isDelayedUiEnabled: boolean }): string | undefined => {
...
export const getAlternateMethodDelayTextOrDefault = (
  ...
  if (trustedSessionCount > 0 && completeDelayTextByMethod) {
    return <React.Fragment>
        ...
        {'\n'}
        {resources.Label.DelayedVerification.TryAgainOnTrustedDeviceSuffix(sessionManagementLinkWithRedirect)}
      </React.Fragment>
  }

  if (trustedSessionCount > 0) {
    return <React.Fragment>
      {resources.Label.DelayedVerification.TryAgainOnTrustedDevicePrefix(sessionManagementLinkWithRedirect)}
      {'\n'}
      {completeDelayTextByMethod}
    </React.Fragment>;
  }
