...
export const loadChallenge = async ({
  ...
  // the modal renders.
  const accountInfoRes = await requestService.legacyAccountInfoApi.getAccountInfo();
  if (
    !accountInfoRes.isError &&
    resultMetadata.value.twoStepCopyTextEnrollmentStatus === TwoStepCopyEnrollmentStateCensored &&
    CurrentUser?.isAuthenticated
  ) {
    const { UserAbove13, UserEmail } = accountInfoRes.value;
    const u13NormalizedCopyThunk = UserAbove13
      ? resources.Label.EnterEmailCodeSanitizedEmail
      : resources.Label.EnterEmailCodeSanitizedEmailU13;

    setUserEmailCopy(u13NormalizedCopyThunk(UserEmail));
  }
