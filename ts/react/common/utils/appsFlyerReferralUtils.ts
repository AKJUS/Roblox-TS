...
export const getAppsFlyerReferralParams = (
  ...
  return Object.entries(AppsFlyerReferralParamsDeeplinkConfig).reduce((acc, [key, value]) => {
    ...
    if (
      firstMatchingKey &&
      (typeof params[firstMatchingKey] === 'string' || typeof params[firstMatchingKey] === 'number')
    ) {
